...
- `tokens/` — per-category token docs (`border-width-tokens.md`, `breakpoint-tokens.md`, `motion-tokens.md`, `spacing-tokens.md`, etc.), plus one category split further into per-family files under `tokens/colors-tokens/`: `color-tokens.md` (family index), `background.md`, `surface.md`, `border.md`, `content.md`, `symbols.md`, `scale.md`, `native.md`, and a draft `surface-border-combination-audit.md` (human-facing only — not yet an AI-facing ruleset, see the file's own header). All filenames are lowercase kebab-case. Every token file follows the same template: an `Overview` table (which family/layer to use — omitted where there's only one), a `Semantic usage` table (which specific token, when), and a `Tokens` table (exact values) — no tool-specific source annotations, so the docs stay usable regardless of what pipes them into Confluence/Figma/elsewhere.
