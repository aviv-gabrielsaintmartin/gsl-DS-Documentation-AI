...
- `figma-icons-registry.json` — flat inventory of Foundations' "Icons" page (455 icons across 17 category frames as of 2026-08-27), each entry's name, Figma key, `nodeId`, variant count, and its own `componentPropertyDefinitions` (no Pattern 1/2 classification — icons are leaf components). Sole source of truth for `figma-sync-icons`.
...
- The four `figma-sync-*` skills never read or write Confluence — the `figma-*-registry.json` files at repo root are their sole source of truth.
